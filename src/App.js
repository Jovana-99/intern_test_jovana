...
function App() {
  ...
  const sortData = (data) => {
    ...
    console.log("sortirano:",sortedData);
    return sortedData;
  }
  ...
  useEffect(()=>{
    ...
    .then(res=>{
      ...
      if(newCustomerView===true){
        var filtereData = res.data.filter(promo=>promo.onlyNewCustomers===true)
        filtereData.sort((a, b) => { return a.sequence - b.sequence; });
        setData(filtereData)
      }else{
